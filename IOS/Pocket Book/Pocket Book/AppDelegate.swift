...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        isFirstAccess = true
        dbName = "NameCard.db"
        dbPath = "/Users/shyoon/Downloads/NameCard.db"
        readNCDataFromDB()
    ...
    func readNCDataFromDB() {
        ...
        if sqlite3_open(dbPath, &database) == SQLITE_OK {
            let sqlStatement: String = "SELECT * FROM tblNameCard ORDER BY NC_INDEX DESC"
            ...
            if sqlite3_prepare_v2(database, sqlStatement, EOF, &compiledStatement, nil) == SQLITE_OK {
                while sqlite3_step(compiledStatement) == SQLITE_ROW {
                    ...
                    let aTel = String(cString: sqlite3_column_text(compiledStatement, 3))
                    let ncData: NCData = NCData(pIndex: aIndex, pName: aName, pNo: aNo, pTel: aTel)
                    dbData.append(ncData)
                    print(aIndex, aName, aNo, aTel) // for debug
                }
       } else {
