...
while True:
    cnvt=int(input("1(comp),2(rev),3(rev_comp),Q(종료): ")) # input = 사용자가 입력한 것을 "문자열"타입으로 반환
    if (str(cnvt)=="Q"):
        break
    elif (cnvt >= 1 and cnvt <= 3): # "문자열"로 받은 cnvt를 int로 변형 후 숫자 범위 설정
    # 기존의 문자형 cnvt 대신 int 형으로 새롭게 변형했음 -> 새로운 변수 만들어 값 대입해야함.
            if (cnvt == 1):
                    rst = comp(src)
            elif (cnvt == 2):
                    rst = rev(src)
